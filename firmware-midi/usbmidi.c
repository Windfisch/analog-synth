...
int current_note = 0x40, retrigger_note = 0, release_note = 0; // FIXME DIRTY HACK
...
static void handle_midi_packet(char pkt[4])
{
	int cable = (pkt[0] & 0xF0) >> 4;
	int usbtype = pkt[0] & 0x0F;

	int miditype = (pkt[1] & 0xF0) >> 4;
	int channel = pkt[1] & 0x0F;
